...
\end{center} \par
焦点就被切换了（你是不是觉得第一个图像上的“OK”按钮也是具有焦点的？错误。这个按钮具有“默认”属性，即此时在这个窗口上按下回车键相当于按下这个键）。Tab键是切换焦点的最佳键。注意，焦点和活动窗口具有唯一性。\par
...
\end{center}\par
HDMI接口。一个较扁的梯形与矩形的结合物（十分类似于一辆车的形状），内部有一块类似于芯片的东西。一种高清显示接口。能传播音频信号。
\begin{center}
...
\begin{center}
	\includegraphics{pic/RJ}\includegraphics[scale=0.2]{pic/RJL}
\end{center}\par
2个3.5mm TRS接口。
\begin{center}
...
\begin{center}
	\includegraphics[scale=0.6]{pic/sg4}	\includegraphics[scale=0.6]{pic/sg5}
\end{center}\par
选择保存位置：
\begin{center}
...
\begin{center}
	\includegraphics[scale=0.7]{pic/kj2}\\\includegraphics{pic/kj3}
\end{center} \par
